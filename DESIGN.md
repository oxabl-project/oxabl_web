...
It is one bordered three-cell control, built from the same parts as everything else on the site: 1px rules, square corners, chrome type, `ink-muted` for the inactive cells, `ink` on `paper-sunk` for the active one. From 640px up it sits at the right end of the bar, where it has always sat. Below 640px it sits inside the menu panel instead — it is the widest object on the bar and the reason the wordmark and the links no longer fit beside it, so leaving it there would not have fixed anything. The component is the same one in both places; what changes inside the panel is the size of its cells. On the bar they are sized for a cursor, which is a smaller target than a thumb can reliably hit, so in the panel each cell takes a 44px minimum. The type, the rules, the colours and the three-state behaviour are untouched — only the hit area grows, and it grows from outside the component so the bar's own instance keeps its metrics exactly.
...
**It comes up from the bottom.** The panel is anchored flush to the bottom edge of the viewport, full width, sized by its contents rather than filling the screen, with a 1px rule along its top edge and enough bottom padding to clear the iOS home indicator. Bottom is where a thumb already is, and it is where this site put the phone menu before the redesign — the pre-redesign `MobileNav.tsx` was a bottom sheet, and that placement was the one part of it worth keeping. Anchored, not sliding: the panel is simply there or not there, so no part of law 4 is bent to justify the position.
...
`MobileNav.tsx` and `ui/sheet.tsx` stay removed, and the menu panel under **Navigation** does not restore them. It occupies the same role, which is exactly why the distinction has to be written down rather than assumed. What was deleted was the shadcn sheet as a _construction_ — rounded corners, a drop shadow, a blurred backdrop, a `--radius` token, an off-canvas slide — and every one of those is still forbidden. The replacement is a native `<dialog>` reset to this site's own box: square, ruled, unblurred, instant. Two things about the old component are deliberately kept — it opened from the bottom of the viewport and it did not repeat the wordmark — because both were right and neither is a shadcn default; they are recorded as rules under **Navigation** so they survive independently of the component that first had them. The role and its position came back; the construction and its defaults did not, and reaching for the library again to fill the role would undo the amendment rather than follow it.
